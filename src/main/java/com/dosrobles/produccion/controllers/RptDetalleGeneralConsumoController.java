...
import com.dosrobles.produccion.reports.detallegeneralconsumo.DetalleGeneralConsumoReport;
...
@Named @ViewScoped
@Getter @Setter
public class RptDetalleGeneralConsumoController implements Serializable {
    ...
    public void generarReporte() {
        ...
        String filename = String.format("rpt_%s_%s.xlsx", "detalle_general_consumo", new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date()));
        response.setHeader("Content-disposition","attachment; filename="+filename);

        try {
            DetalleGeneralConsumoReport.createReportFrom(getOrdenProduccionList(), fechaInicial, fechaFinal, response.getOutputStream());
        } catch (IOException e) {
